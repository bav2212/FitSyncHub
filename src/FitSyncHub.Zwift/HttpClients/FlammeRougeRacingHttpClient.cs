...
internal class FlammeRougeRacingHttpClient : IFlammeRougeRacingHttpClient
{
    ...
    public async Task<List<long>> GetTourRegisteredRiders(FlammeRougeRacingCategory flammeRougeRacingCategory, CancellationToken cancellationToken)
    {
        ...
        List<long> riderIds = [];
        await foreach (var riderData in PullRiderData(TableIdTourRegistered, formData, cancellationToken))
        {
    ...
    public async Task<List<FlammeRougeRacingEGapResultModel>> GetYellowJerseyStandings(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = new List<FlammeRougeRacingEGapResultModel>();
        await foreach (var riderData in PullRiderData(TableIdTourResultsGC, formData, cancellationToken))
        {
        ...
        CancellationToken cancellationToken)
    {
        var formData = new Dictionary<string, string>()
        {
            ...
            {"columns[0][search][regex]", "false"},
            {"columns[1][data]", "1"},
            {"columns[1][name]", "STAGE"},
            {"columns[1][searchable]", "true"},
            {"columns[1][orderable]", "false"},
            {"columns[1][search][value]", $"{stageNumber}"},
            {"columns[1][search][regex]", "true"},
            {"columns[2][data]", "2"},
            {"columns[2][name]", "EVENTREF"},
            {"columns[2][searchable]", "true"},
            {"columns[2][orderable]", "false"},
            ...
            {"columns[3][searchable]", "true"},
            {"columns[3][orderable]", "false"},
            {"columns[3][search][value]", $"M-{flammeRougeRacingCategory}"},
            {"columns[3][search][regex]", "true"},
            {"columns[4][data]", "4"},
            {"columns[4][name]", "GENDER"},
            {"columns[4][searchable]", "true"},
            ...
            {"columns[6][search][regex]", "false"},
            {"columns[7][data]", "7"},
            {"columns[7][name]", "RCLASS"},
            {"columns[7][searchable]", "true"},
            {"columns[7][orderable]", "false"},
            {"columns[7][search][value]", ""},
            {"columns[7][search][regex]", "false"},
            {"columns[8][data]", "8"},
            {"columns[8][name]", "POSITION"},
            {"columns[8][searchable]", "true"},
            {"columns[8][orderable]", "false"},
            {"columns[8][search][value]", ""},
            {"columns[8][search][regex]", "false"},
            {"columns[9][data]", "9"},
            {"columns[9][name]", "RIDER"},
            {"columns[9][searchable]", "true"},
            {"columns[9][orderable]", "false"},
            {"columns[9][search][value]", ""},
            {"columns[9][search][regex]", "false"},
            {"columns[10][data]", "10"},
            {"columns[10][name]", "RIDERID"},
            {"columns[10][searchable]", "true"},
            {"columns[10][orderable]", "false"},
            {"columns[10][search][value]", ""},
            {"columns[10][search][regex]", "false"},
            {"columns[11][data]", "11"},
            {"columns[11][name]", "CLUB"},
            {"columns[11][searchable]", "true"},
            {"columns[11][orderable]", "false"},
            {"columns[11][search][value]", ""},
            {"columns[11][search][regex]", "false"},
            ...
            {"columns[13][search][regex]", "false"},
            {"columns[14][data]", "14"},
            {"columns[14][name]", "STAGES"},
            {"columns[14][searchable]", "true"},
            {"columns[14][orderable]", "false"},
            {"columns[14][search][value]", ""},
            {"columns[14][search][regex]", "false"},
            {"columns[15][data]", "15"},
            {"columns[15][name]", "POINTS"},
            {"columns[15][searchable]", "true"},
            {"columns[15][orderable]", "false"},
            {"columns[15][search][value]", ""},
            {"columns[15][search][regex]", "false"},
        ...
        await foreach (var riderData in PullRiderData(TableIdTourResultsPolkaDot, formData, cancellationToken))
        {
            ...
            var frrResult = new FlammeRougeRacingPointsResultModel
            {
                Position = int.Parse(riderDataArray[8].GetString()!, NumberStyles.AllowThousands, CultureInfo.InvariantCulture),
                Rider = riderDataArray[9].GetString()!,
                RiderId = long.Parse(riderDataArray[10].GetString()!, NumberStyles.AllowThousands, CultureInfo.InvariantCulture),
                TotalPoints = int.Parse(riderDataArray[18].GetString()!, NumberStyles.AllowThousands, CultureInfo.InvariantCulture),
            };
            result.Add(frrResult);
        }

        return result;
    }

    public async Task<List<FlammeRougeRacingPointsResultModel>> GetGreenJerseyStandings(
        FlammeRougeRacingCategory flammeRougeRacingCategory,
        int stageNumber,
        CancellationToken cancellationToken)
    {
        ...
        await foreach (var riderData in PullRiderData(TableIdTourResultsGreenJersey, formData, cancellationToken))
        {
            ...
            var frrResult = new FlammeRougeRacingPointsResultModel
            {
                Position = int.Parse(riderDataArray[7].GetString()!, NumberStyles.AllowThousands, CultureInfo.InvariantCulture),
                Rider = riderDataArray[8].GetString()!,
                RiderId = long.Parse(riderDataArray[9].GetString()!, NumberStyles.AllowThousands, CultureInfo.InvariantCulture),
                TotalPoints = int.Parse(riderDataArray[18].GetString()!, NumberStyles.AllowThousands, CultureInfo.InvariantCulture),
            };
            result.Add(frrResult);
        }
       ...
       [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            var formDataCommon = new Dictionary<string, string>()
            {
                {"start", $"{start}" },
                {"length", $"{Page}" },
                {"wdtNonce", wdtNonce },
            ...
            var formDataCombined = formData
                .Concat(formDataCommon)
                .ToDictionary(kvp => kvp.Key, kvp => kvp.Value);

            var dataContent = new FormUrlEncodedContent(formDataCombined);
