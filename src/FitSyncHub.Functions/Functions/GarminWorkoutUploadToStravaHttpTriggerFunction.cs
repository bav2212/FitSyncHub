...
public class GarminWorkoutUploadToStravaHttpTriggerFunction
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var workoutActivity in activities)
        {
            ...
            var uploadStartResponse = await _stravaRestHttpClient.UploadStart(uploadFileModel, uploadModel, cancellationToken);
            _logger.LogInformation("Upload started with id {UploadId}", uploadStartResponse.Id);

            var uploadResponse = await _stravaRestHttpClient.GetUpload(uploadStartResponse.Id, cancellationToken);
